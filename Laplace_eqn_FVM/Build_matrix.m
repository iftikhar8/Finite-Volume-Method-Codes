...
%Compute a_e and its effects             
        ...
        %Update coefficients in the A matrix:
        %Note row=centre and others are columns:
        
        %Access different element numbers and fill the matrix:
        centre=element_number(i,j);  A(centre,centre)=a_p;
        if i>1
        north=element_number(i-1,j); A(centre,north)=a_n;
        end
        
        if i<Ny
        south=element_number(i+1,j);  A(centre,south)=a_s;
        end
        
        if j<Nx
        east=element_number(i,j+1);   A(centre,east)=a_e;
        end
        
        if j>1
        west=element_number(i,j-1);   A(centre,west)=a_w; 
        end
